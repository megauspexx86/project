...
class PopupRemoveComponentFromArchive extends React.Component {

    removeComponentFromArchive() {
        let r = new Request(`/json/component/restore/${this.props.component_id}`);
        r.send(((response) => {
    ...
    render() {
            ...
            return(
                <Dialog name="popup_remove_component_from_archive">
                    ...
                    <div className="popup__body cancel-order">
                        <p className="cancel-order__message">{`Вы действительно хотите восстановить промо-материал «${this.props.title}»?`}</p>
                        <nav className="cancel-order__navigation cancel-order__navigation_centred">
                            <button  className="authors__select btn ui-btn ui-btn_lg ui-btn_blue" onClick={(() => {this.removeComponentFromArchive()}).bind(this)}>Да</button>
    ...
    _close() {
        setTimeout((() => {
            ReactDOM.unmountComponentAtNode(document.getElementById('popup_remove_component_from_archive'));
        }).bind(this), 0.00001);
    }

}

export default PopupRemoveComponentFromArchive;
